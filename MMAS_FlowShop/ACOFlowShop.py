...
class ACOFlowShop:
    ...
    def __init__(self,filepath=None, parameters=None):
        ...
        self.maximum_pheromone = 1/((1-self.p)*self.z_best)
        self.minimum_pheromone = self.maximum_pheromone/5
        self.pheromone_trails = [[self.maximum_pheromone for _ in range(self.number_jobs)] for _ in range(self.number_jobs)]
        ...
        # print(DataFrame(self.pheromone_trails))
    ...
    def update_trail_intensities(self, ant):
        ...
        for i in range(len(self.pheromone_trails)):
            for k in range(len(self.pheromone_trails)):
                ...
                if sequence.index(i) == k:
                    new_value = self.p*old_value + (1/z_current)
                else:
                    new_value = self.p*old_value

                if new_value> self.maximum_pheromone:
                    new_value = self.maximum_pheromone
                elif new_value < self.minimum_pheromone:
                    new_value = self.minimum_pheromone
    ...
    def clear_solution(self):
        ...
        self.maximum_pheromone = 1 / ((1 - self.p) * self.z_best)
        self.minimum_pheromone = self.maximum_pheromone / 5
        self.pheromone_trails = [[self.maximum_pheromone for _ in range(self.number_jobs)] for _ in
                                 range(self.number_jobs)]
    ...
    def show_graphics(self):
        plt.title(('Flowshop ACO (M-MMAS): {} trabajos, {} máquinas ').format(self.number_jobs,self.number_machines) )
        ...
        plt.annotate('Min Z =' + str(min_z), xy=(x_min_z,min_z),  xytext=(x_min_z-2,min_z-12),arrowprops=dict(arrowstyle="->"))
